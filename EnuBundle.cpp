...
// MAKE CHANGE HERE SO I CAN JUST HAVE IT READ .GRAPH FILES //
int EnuBundle::readGraph(const char* filepath) {
...
ui EnuBundle::markBlock1(ui v, ui* adress) {
	...
	ui v_tri_count = 0;
	// adds second neighbors to block
	for (ui i = 0; i < szc1; i++) {
		ui u = cache1[i];
		//for (auto u:nei1s){
		for (ui k = pstart[u]; k < pstart[u + 1]; k++) {
			...
			else if (nei2 != v && dist[nei2] == 1 && dpos[nei2] > dpos[u])
				v_tri_count++;
		}
	}
	...
	for (ui i = 0; i < szc2; i++) {
			...
			if (edges[p1] < edges[p2]) {
				...
				if (dist[edges[p1]] ) {
					...
					if (dist[edges[p1]] == 1) {	
						...
						while (p1_temp != pstart[v + 1] && p2_temp != pstart[u + 1] && p3 != pstart[w + 1]) {
							// std::cout << "top" << std::endl;
							if (edges[p1_temp] == edges[p2_temp] && edges[p1_temp] == edges[p3]) {
								cnt_4clq++;
							...
							else if (edges[p1_temp] <= edges[p2_temp] && edges[p1_temp] <= edges[p3]) {
								p1_temp++;
							}
							else if (edges[p2_temp] <= edges[p1_temp] && edges[p2_temp] <= edges[p3]) {
								p2_temp++;
							}
							else {
								p3++;
		...
		// std::cout << "4-clq count: " << cnt_4clq << std::endl;
		if (dist[u] == 1) {
			// this is my edge triangle reduction -> kinda
			if (cnt_tri + 2 * k < lb) {
				dist[u] = 0;
				//canprune = 1;
				szblk--;
...
#ifdef EDGE4CLQ
			// 4-clique edge reduction
			else if ( 2 * cnt_4clq < (lb - 2 * k) * (lb - 3 * k)) {
				dist[u] = 0;
				szblk--;
				e_skipped++;
...
int EnuBundle::canMoveToP(ui u) {
	...
	for (ui i = 0; i < P.getSize(); i++) {
		...
		// some vertex is already missing k vertices in P, and u is not in the neighborhood of that vertex
...
*/
 void EnuBundle::branch() {
	...
	*/
	...
	// minu is the vertex with the smallest degree in P
	// if minu is missing neighbors in Cand or P
	if (minu != UINT_MAX && neiInG[minu] + k < Cand.getSize() + P.getSize()) { // The whole graph can not be a k-plex
		ui szmax = k + neiInP[minu] - P.getSize(); // gets the number of disconnected vertices you can add
		vector<ui> doing;
		for (ui i = 0; i < Cand.getSize(); i++) {
			ui u = Cand.get(i);
	...
	if (decompose) { // decompose the graph and enumerate k-plexes in smaller subgraphs
		...
		for (ui i = 0; i < n - 1; i++) {
			...
			if (core[v] + k >= lb) {
				...
				// Cand starts as every thing in the block (the subgraph)
